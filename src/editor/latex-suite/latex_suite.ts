...
export const handleUpdate = (update: ViewUpdate) => {
	...
	// Math tooltip / inline preview is not ported in this build.
...
export const keyboardEventPlugin = ViewPlugin.fromClass(class {
	...
	onKeydown(event: KeyboardEvent, view: EditorView): boolean {
		...
		// Returning true tells CodeMirror the event was handled, so it won't
		// fall through to other keydown handlers (e.g. a Tab indent binding).
		return success;
	}
}, {
	eventHandlers: {
		keydown(event, view): boolean {
			return view.plugin(keyboardEventPlugin)!.onKeydown(event, view);
		},
	},
